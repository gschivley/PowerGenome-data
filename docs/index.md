...
| `build_hydro_profiles.py` | `existing_resource_groups/hydro_run_of_river_2007_2013.parquet`, `existing_resource_groups/hydro_conventional_2007_2013.parquet` | `data/plant_region_map.csv`, `data/reeds_generators_transformed.csv`; PUDL monthly generators parquet (`s3://pudl.catalyst.coop/nightly/out_eia__monthly_generators.parquet`) | Derives monthly capacity factors per region/type, interpolates to hourly, smooths with a 1-week window. |
...
| `build_new_pg_dg_inputs.py` | `distributed_capacity.parquet`, `distributed_profiles.parquet` | ReEDS (ReEDS-Model/ReEDS) `dgen_model_inputs/stscen2023_mid_case/distpvcap_stscen2023_mid_case.csv`; `zones/z134/county2zone.csv`; `disaggregation/county_population.csv`; hourly county-level distpv CF HDF5 (`cf_distpv_county.h5`, hosted on Zenodo and resolved via ReEDS `remote_files.csv`) | County-level inputs are aggregated to ReEDS BA regions: county capacity is summed and converted from DC to AC (ILR=1.1); county capacity-factor profiles are averaged per BA (capacity-weighted by default, or population-weighted via `--weight-by`). Profiles download can be skipped via `--skip-profiles`; cache the large HDF5 between runs with `--cache-dir`. |
...
uv run python build_new_pg_dg_inputs.py --skip-profiles  # avoid large HDF5 download
# Full run (caches the large HDF5 under cache_data/ for reuse):
uv run python build_new_pg_dg_inputs.py --cache-dir cache_data
